...
class Fusion(object):
    ...
    def update_nomag(self, accel, gyro):    # 3-tuples (x, y, z) for accel, gyro
        ...
        if self.start_time is None:
            #self.start_time = pyb.micros()  #  First run first adjustment
            self.start_time = time_module.micros()
        ...
        deltat = time_module.elapsed_micros(self.start_time) / 1000000
        self.start_time = time_module.micros()
        q1 += qDot1 * deltat
        q2 += qDot2 * deltat
        q3 += qDot3 * deltat
    ...
    def update(self, accel, gyro, mag):     # 3-tuples (x, y, z) for accel, gyro and mag data
        mx, my, mz = (float(mag[x]) - self.magbias[x] for x in range(3)) # Units irrelevant (normalised)
        ax, ay, az = accel                  # Units irrelevant (normalised)
        gx, gy, gz = (radians(x) for x in gyro)  # Units deg/s
        if self.start_time is None:
            self.start_time = time_module.micros()  # First run
        q1, q2, q3, q4 = (self.q[x] for x in range(4))   # short name local variable for readability
        # Auxiliary variables to avoid repeated arithmetic
        _2q1 = 2 * q1
        _2q2 = 2 * q2
